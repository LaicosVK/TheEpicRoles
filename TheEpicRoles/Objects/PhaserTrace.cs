...
namespace TheEpicRoles.Objects {
    class PhaserTrace {
        ...
        public static Sprite getTraceSprite() {
            ...
            TraceSprite = Helpers.loadSpriteFromResources("TheEpicRoles.Resources.PhaserTraceW.png", 225f);
            return TraceSprite;
        }
        ...
        public PhaserTrace(Vector2 p, float duration=1f) {
            ...
            float colorDuration = CustomOptionHolder.phaserTraceColorTime.getFloat();
            FastDestroyableSingleton<HudManager>.Instance.StartCoroutine(Effects.Lerp(colorDuration, new Action<float>((p) => {
                ...
                if (Helpers.isLighterColor(Phaser.phaser.Data.DefaultOutfit.ColorId)) c = Color.white;
        ...
        public static void UpdateAll() {
            foreach (PhaserTrace traceCurrent in new List<PhaserTrace>(traces))
            {
