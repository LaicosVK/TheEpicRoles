...
public class BepInExUpdater : MonoBehaviour
{
    ...
    public void Awake()
    {
        TheEpicRolesPlugin.Logger.LogMessage("BepInEx Update Required...");
    ...
    {
        Task.Run(() => MessageBox(IntPtr.Zero, "Required BepInEx update is downloading, please wait...","The Epic Roles", 0));
        UnityWebRequest www = UnityWebRequest.Get(BepInExDownloadURL);
        yield return www.Send();        
        if (www.isNetworkError || www.isHttpError)
        {
            TheEpicRolesPlugin.Logger.LogError(www.error);
            yield break;
        }
