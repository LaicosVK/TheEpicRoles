...
namespace TheEpicRoles.Modules 
{
    public class ModUpdateBehaviour : MonoBehaviour
    {
        ...
        private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
        {
            ...
            var announcement = $"<size=150%>A new <color=#FC0303>{(isSubmerged ? "Submerged" : "THE EPIC ROLES")}</color> update to {(isSubmerged ? SubmergedUpdate.Tag : TORUpdate.Tag)} is available</size>\n{(isSubmerged ? SubmergedUpdate.Content : TORUpdate.Content)}";
        ...
        public IEnumerator CoUpdate()
        {
            ...
            var updateName = (isSubmerged ? "Submerged" : "The Epic Roles");
        ...
        {
            var torUpdateCheck = Task.Run(() => Instance.GetGithubUpdate("LaicosVK", "TheEpicRoles"));
            while (!torUpdateCheck.IsCompleted) yield return null;
            Announcement.updateData = torUpdateCheck.Result;
            if (torUpdateCheck.Result != null && torUpdateCheck.Result.IsNewer(Version.Parse(TheEpicRolesPlugin.VersionString)))
            {
        ...
        public async Task<UpdateData> GetGithubUpdate(string owner, string repo)
        {
            ...
            client.DefaultRequestHeaders.Add("User-Agent", "TheEpicRoles Updater");
        ...
        private bool TryUpdateSubmergedInternally()
        {
            ...
            {
                TheEpicRolesPlugin.Logger.LogError(e);
                return false;
            }
            return true;
        }
        ...
        public async Task<bool> DownloadUpdate()
        {
            ...
            string filePath = Path.Combine(Paths.PluginPath, isSubmerged ? "Submerged.dll" : "TheEpicRoles.dll");
