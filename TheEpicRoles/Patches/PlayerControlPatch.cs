...
namespace TheEpicRoles.Patches {
    ...
    public static class PlayerControlFixedUpdatePatch {
        ...
        static void setPlayerOutline(PlayerControl target, Color color) {
            ...
            target.cosmetics.skin.layer.material.SetFloat("_Outline", 1f);
            target.cosmetics.skin.layer.material.SetColor("_OutlineColor", color);
        }
        ...
        static void setBasePlayerOutlines() {
            foreach (PlayerControl target in CachedPlayer.AllPlayers) {
                if (target == null || target.cosmetics == null) continue;
                ...
                target.cosmetics.skin.layer.material.SetColor("_OutlineColor", color);
                    ...
                    target.cosmetics.skin.layer.material.SetFloat("_Outline", 0f);
                }
            }
        }
        ...
        static void phaserUpdate()
        {
            if (Phaser.isInvisble && Phaser.invisibleTimer <= 0 && Phaser.phaser == CachedPlayer.LocalPlayer.PlayerControl)
            {
                ...
                RPCProcedure.setInvisible(Phaser.phaser.PlayerId, byte.MaxValue);
            }
            if (Phaser.arrow?.arrow != null)
            {
                if (Phaser.phaser == null || Phaser.phaser != CachedPlayer.LocalPlayer.PlayerControl || !Phaser.knowsTargetLocation) {
                    Phaser.arrow.arrow.SetActive(false);
                    return;
                }
                if (Phaser.phaserMarked != null && !CachedPlayer.LocalPlayer.Data.IsDead)
                {
                    bool trackedOnMap = !Phaser.phaserMarked.Data.IsDead;
                    Vector3 position = Phaser.phaserMarked.transform.position;
                    if (!trackedOnMap)
                    { // Check for dead body
                        DeadBody body = UnityEngine.Object.FindObjectsOfType<DeadBody>().FirstOrDefault(b => b.ParentId == Phaser.phaserMarked.PlayerId);
                        if (body != null)
                        {
                            trackedOnMap = true;
        ...
        {
            if (Phaser.phaser == null || Phaser.phaser != CachedPlayer.LocalPlayer.PlayerControl) return;
            List<PlayerControl> untargetables = new List<PlayerControl>();
            ...
            Phaser.currentTarget = setTarget(onlyCrewmates: true, untargetablePlayers: untargetables);
    ...
    public static class MurderPlayerPatch
    {
        ...
        public static void Postfix(PlayerControl __instance, [HarmonyArgument(0)]PlayerControl target)
        {
            ...
            // Phaser Button Sync
            if (Phaser.phaser != null && CachedPlayer.LocalPlayer.PlayerControl == Phaser.phaser && __instance == Phaser.phaser && HudManagerStartPatch.phaserButton != null)
                HudManagerStartPatch.phaserButton.Timer = HudManagerStartPatch.phaserButton.MaxTimer;
    ...
    class KillAnimationSetMovementPatch {
        ...
        public static void Prefix(PlayerControl source, bool canMove) {
            Color color = source.cosmetics.skin.layer.material.GetColor("_BodyColor");
