        ...
        public static bool Prefix(KillButton __instance) {
            if (__instance.isActiveAndEnabled && __instance.currentTarget && !__instance.isCoolingDown && !CachedPlayer.LocalPlayer.Data.IsDead && CachedPlayer.LocalPlayer.PlayerControl.CanMove) {
                ...
                if (res == MurderAttemptResult.BlankKill) {
                    ...
                    else if (CachedPlayer.LocalPlayer.PlayerControl == Phaser.phaser)
                        Phaser.phaser.killTimer = HudManagerStartPatch.phaserButton.Timer = HudManagerStartPatch.phaserButton.MaxTimer;
                }
                        ...
                        if (plainShipRoom != null && plainShipRoom.roomArea)
                        {
                            ...
                            for (int j = 0; j < num; j++)
                            {
                                ...
                                if (!(collider2D.tag == "DeadBody"))
                                {
                                    ...
                                    {
                                        num2--;
                                    } else if (component?.cosmetics?.skin.layer.material != null) {
                                        Color color = component.cosmetics.skin.layer.material.GetColor("_BodyColor");
